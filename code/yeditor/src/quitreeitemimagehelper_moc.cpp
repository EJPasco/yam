/****************************************************************************
** Meta object code from reading C++ file 'quitreeitemimagehelper.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.3.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../inc/quitreeitemimagehelper.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'quitreeitemimagehelper.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.3.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_YCQUiTreeItemImageHelper_t {
    QByteArrayData data[12];
    char stringdata[177];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_YCQUiTreeItemImageHelper_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_YCQUiTreeItemImageHelper_t qt_meta_stringdata_YCQUiTreeItemImageHelper = {
    {
QT_MOC_LITERAL(0, 0, 24),
QT_MOC_LITERAL(1, 25, 9),
QT_MOC_LITERAL(2, 35, 0),
QT_MOC_LITERAL(3, 36, 11),
QT_MOC_LITERAL(4, 48, 7),
QT_MOC_LITERAL(5, 56, 25),
QT_MOC_LITERAL(6, 82, 12),
QT_MOC_LITERAL(7, 95, 19),
QT_MOC_LITERAL(8, 115, 13),
QT_MOC_LITERAL(9, 129, 19),
QT_MOC_LITERAL(10, 149, 8),
QT_MOC_LITERAL(11, 158, 18)
    },
    "YCQUiTreeItemImageHelper\0onChanged\0\0"
    "yam::yint32\0riIndex\0YCQUiTreeItemImageHelper*\0"
    "pImageHelper\0onItemChangedSource\0"
    "rsImageSource\0onItemChangedOffset\0"
    "roOffset\0onDelButtonClicked"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_YCQUiTreeItemImageHelper[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    2,   39,    2, 0x06 /* Public */,
       1,    1,   44,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       7,    1,   47,    2, 0x0a /* Public */,
       9,    1,   50,    2, 0x0a /* Public */,
      11,    0,   53,    2, 0x0a /* Public */,

 // signals: parameters
    QMetaType::Void, 0x80000000 | 3, 0x80000000 | 5,    4,    6,
    QMetaType::Void, 0x80000000 | 3,    4,

 // slots: parameters
    QMetaType::Void, QMetaType::QString,    8,
    QMetaType::Void, QMetaType::QPoint,   10,
    QMetaType::Void,

       0        // eod
};

void YCQUiTreeItemImageHelper::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        YCQUiTreeItemImageHelper *_t = static_cast<YCQUiTreeItemImageHelper *>(_o);
        switch (_id) {
        case 0: _t->onChanged((*reinterpret_cast< const yam::yint32(*)>(_a[1])),(*reinterpret_cast< YCQUiTreeItemImageHelper*(*)>(_a[2]))); break;
        case 1: _t->onChanged((*reinterpret_cast< const yam::yint32(*)>(_a[1]))); break;
        case 2: _t->onItemChangedSource((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 3: _t->onItemChangedOffset((*reinterpret_cast< const QPoint(*)>(_a[1]))); break;
        case 4: _t->onDelButtonClicked(); break;
        default: ;
        }
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        switch (_id) {
        default: *reinterpret_cast<int*>(_a[0]) = -1; break;
        case 0:
            switch (*reinterpret_cast<int*>(_a[1])) {
            default: *reinterpret_cast<int*>(_a[0]) = -1; break;
            case 1:
                *reinterpret_cast<int*>(_a[0]) = qRegisterMetaType< YCQUiTreeItemImageHelper* >(); break;
            }
            break;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (YCQUiTreeItemImageHelper::*_t)(const yam::yint32 & , YCQUiTreeItemImageHelper * );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&YCQUiTreeItemImageHelper::onChanged)) {
                *result = 0;
            }
        }
        {
            typedef void (YCQUiTreeItemImageHelper::*_t)(const yam::yint32 & );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&YCQUiTreeItemImageHelper::onChanged)) {
                *result = 1;
            }
        }
    }
}

const QMetaObject YCQUiTreeItemImageHelper::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_YCQUiTreeItemImageHelper.data,
      qt_meta_data_YCQUiTreeItemImageHelper,  qt_static_metacall, 0, 0}
};


const QMetaObject *YCQUiTreeItemImageHelper::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *YCQUiTreeItemImageHelper::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_YCQUiTreeItemImageHelper.stringdata))
        return static_cast<void*>(const_cast< YCQUiTreeItemImageHelper*>(this));
    return QObject::qt_metacast(_clname);
}

int YCQUiTreeItemImageHelper::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void YCQUiTreeItemImageHelper::onChanged(const yam::yint32 & _t1, YCQUiTreeItemImageHelper * _t2)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void YCQUiTreeItemImageHelper::onChanged(const yam::yint32 & _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}
QT_END_MOC_NAMESPACE
